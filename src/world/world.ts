...
export interface BackdropSpec {
   ...
   *
   * Положение и ширина — ДОЛИ высоты поля звёзд, а не строки: поле выведено
   * из окна неба конкретного мира, и число строк, верное для одного мира
   * и одного кадра, для другого вывело бы полосу за пределы неба.
   */
  readonly milkyWay: {
    /** Доля высоты поля: 0 — верх неба, 1 — линия поверхности. */
    readonly centerY: number;
    /** Полуширина полосы в долях высоты поля. */
...
export interface WorldProfile {
  ...
  /**
   * Ступень пещеры в глубине массива, темнее `caveColor`. Ею выражается
   * удаление от выхода: плоская заливка сообщает только «здесь пусто»,
   * затенённая — ещё и «выход в той стороне».
   *
   * Инвариант: светлее любого твёрдого материала быть не может — пустота
   * обязана читаться пустотой при любом затенении.
   */
  readonly caveDeepColor: number;
