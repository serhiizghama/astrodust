...
export class Simulation {
   ...
   * ```
   *   вниз?    ─── занято
   *   спуск по столбику подо мной, не глубже MAX_CARRY_DEPTH:
   *       пустота  ─── разрыв: я не лежу на поверхности, диагональ
   *       несущая  ─── d ≤ её глубины → вбок по carry, и диагонали больше нет
   *                    d > её глубины → я выше полосы, диагональ
   *   иначе диагональ вниз-вбок?
   * ```
   *
   * Инвариант: перенос ВЫТЕСНЯЕТ диагональ, а не дополняет — ни вместо неё,
   * ни после отказа, и не важно, каким рядом полосы идёт ячейка. Иначе лента
   * течёт: часть груза уходит по диагоналям на пол, очередь на ленте
   * становится невозможна, а очередь и есть то, чем лента отличается
   * от жёлоба. Выше полосы груз лежит уже не на ленте и подчиняется обычным
   * правилам.
   *
  ...
  private movePowder(
    ...
    // Спуск идёт ОТ ГРУЗА ВНИЗ: правило принадлежит грузу. Лента статична,
    // автомат её не обходит вовсе — начнись обход от поверхности, пустая лента
    // будила бы чанки фактом существования.
    //
    // Столбик обрывается первой же пустотой: ячейка, под которой воздух,
    // на поверхности не лежит, и тянуть её значило бы притягивать падающее.
    // Поэтому улёгшаяся на породе куча платит одно чтение, как и прежде.
    let carry = 0;
    let band = 0;
    let row = 0;
    for (let d = 1; d <= MAX_CARRY_DEPTH; d++) {
      const below = world.get(x, y + d);
      if (below === MAT.VACUUM) break;
      const depth = MAT_CARRY_DEPTH[below]!;
      if (depth !== 0) {
        // Глубину решает найденная поверхность, а не автомат: выше своей
        // полосы она не тянет ничего.
        if (d <= depth) {
          carry = MAT_CARRY[below]!;
          band = depth;
          row = d;
        }
        break;
    ...
    if (carry !== 0) {
      // Полосу везёт её НИЖНЯЯ сыпучая ячейка, и увозит столбик над собой одним
      // движением; верхние ряды молча возвращают управление.
      //
      // Инвариант порядка: обход идёт снизу вверх, и порядно полоса
      // ОБВАЛИВАЛАСЬ бы — уехавшая нижняя ячейка оставляет под верхней дыру,
      // и та падает в неё вместо того, чтобы ехать. Ряд, уехавший вместе
      // с ведущей ячейкой, попадает в уже пройденную строку и второй раз
      // за шаг не трогается. Держит `tests/material-simulation.ts`.
      //
      // Возврат отсюда идёт ДО осыпания — запрет диагонали этим и накрывает
      // всю полосу, а не только её нижний ряд.
      if (MAT_STATE[world.get(x, y + 1)] === MatterState.Powder) return -1;
      ...
      if (!this.tryMove(world, occupant, x, y, x + carry, y, material)) return -1;

      // Столбик над ведущей ячейкой — до конца полосы и до первого несыпучего.
      // Ведущая уже уехала, поэтому дыры под этими рядами нет только пока они
      // едут следом: остановленный ряд обрывает перенос столбика, и всё, что
      // над ним, доедет своим ходом на следующем шаге.
      for (let k = 1; k <= band - row; k++) {
        const above = world.get(x, y - k);
        if (MAT_STATE[above] !== MatterState.Powder) break;
        if (!this.tryMove(world, occupant, x, y - k, x + carry, y - k, above)) break;
      }
      return x + carry;
    }
