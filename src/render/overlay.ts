...
export function drawResearchOverlay(
  ...
  // плавающая коробка закрывает соседние узлы и связи, то есть ровно то, ради
  // чего дерево и рисуется графом.
  //
  // Источник — наведённый узел, иначе выбранный: курсор сходит с узла
  // постоянно, и полоса, пустеющая на каждое такое движение, мигает вместо
  // того, чтобы отвечать.
