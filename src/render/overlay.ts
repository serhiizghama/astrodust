 ...
 *
 * Рисуется ТЕМ ЖЕ контекстом канваса, что и строка состояния, — то есть после
 * `display.present()`, поверх уже выведенного буфера. Отдельного слоя DOM нет
 * и быть не должно: второй слой в другом разрешении — это второй масштаб
 * на экране, а целочисленный апскейл всего кадра записан требованием оболочки,
 * и интерфейс, живущий вне канваса, из-под него выпадает.
 *
 * Подложка НЕПРОЗРАЧНАЯ. Мир под оверлеем любой — от чёрной пещеры до светлого
 * реголита, — и читаемость текста на произвольном фоне не обеспечивается ничем,
 * кроме подложки.
 *
 * Дерево показывается СПИСКОМ, а не графом: граф со связями требует раскладки,
 * а раскладка — отдельная работа ради того же ответа (что открыто, что доступно,
 * что чего требует). Предпосылка пишется текстом в строке.
 */
