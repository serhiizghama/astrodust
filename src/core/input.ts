...
/**
 * Ввод, в котором ничего не нажато.
 *
 * Персонаж получает ИМЕННО ЕГО, пока ввод принадлежит меню, — а не пропускает
 * шаг целиком: физика обязана продолжать работать (застигнутый в воздухе игрок
 * должен падать, а не зависать), но намерений игрока в ней быть не должно.
 * Ветка «а если открыто меню» внутри самого персонажа означала бы, что о меню
 * знает то, что от него дальше всего.
 */
 ...
 * режим инструмента `R`, выбранное вещество `C`, вид постройки `X`,
 * оверлей исследований `T`, прицел вниз `S` (`↓`).
 * Обе половины — WASD и стрелки — равноправны, и игра полностью проходится
 * без мыши: у каждой кнопки мыши есть клавиша.
 *
 * Пока оверлей исследований открыт, та же раскладка означает другое: клавиши
 * направления выбирают строку списка, применение инструмента покупает. Снапшот
 * этого не знает — он сообщает нажатия, а кому они достанутся, решает игровой
 * цикл. Двусмысленности нет: открытый оверлей виден.
 */
export class Input {
