...
import { Renderer, BRUSH_OUTLINE, VACUUM_OUTLINE, stripeOffset } from '../src/render/renderer';
...
// Поэтому проверки идут по свойству переноса, а не по идентификатору ленты.
{
  ...
  function lay(
    ...
    return Builder.apply(w, registry, module, kind, x, y, x, y);
  }
  ...
  {
    const state = new BuildCatalogState();
...

  {
    ...
    check(
      ...
      Builder.issueAt(w, CONVEYOR_RIGHT_KIND, 50, 20, 1000) === null &&
        Builder.issueAt(w, SEPARATOR_KIND, 50, 20, 1000) === 'unsupported',
    );
  }

  {
    ...
    // на результат не влияет.
    {
      ...
      const results = corners.map(([tx, ty]) =>
        Builder.apply(w2, r2, m2, CONVEYOR_RIGHT_KIND, tx!, ty!, tx!, ty!),
      );
      check(
...

  {
    ...
    for (let i = 0; i < STEP * 120; i++) {
      sim.update(w, null);
      if (module.update(w) > 0 && earnedAt < 0) earnedAt = i;
    }
    let slagInZone = 0;
    ...
    check(
      ...
      module.credits === MAT_CREDITS[MAT.IRIDIUM] && count(w, MAT.IRIDIUM) === 0,
...

  {
    ...
    check(
      ...
      built === 'placed' &&
        module.credits - before === MAT_CREDITS[MAT.IRIDIUM]! &&
        count(w, MAT.IRIDIUM) === 0 &&
  ...
  // применить несколько раз подряд, снести. Ни одного события мыши.
  {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (input.toolPressed && tool.building) {
        const at = 20 + i * size;
        if (Builder.apply(w, registry, module, catalog.kind, at, 32, at, 32) === 'placed') {
          laid++;
        }
      }
      up('Space');
    ...
    const removed =
      input.toolPressed &&
      Builder.apply(w, registry, module, catalog.kind, mid + 1, 33, mid + 1, 33) === 'demolished';
    up('Space');
    input.endStep();
